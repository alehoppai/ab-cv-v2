...
export class StarField {
    ...
    constructor(scene: THREE.Scene, modelCenter: THREE.Vector3, camera: THREE.Camera, numStars = 15000) {
        this.scene = scene;
    ...
    private generateStars() {
        ...
        const starMaterial = new THREE.PointsMaterial({ color: 0xFFFFFF, size: 0.01, transparent: true, opacity: 0.5 });
        ...
        for (let i = 0; i < this.numStars; i++) {
            ...
            const vector = new THREE.Vector3(
                ...
                (Math.random() - 0.5) * 15,
                (Math.random() - 0.5) * 15 
            ).normalize().multiplyScalar(this.sphereRadius * Math.random());  // Random distance from the center
    ...
    public update() {
        if (this.previousMousePos.x !== this.mousePos.x || this.previousMousePos.y !== this.mousePos.y) {
